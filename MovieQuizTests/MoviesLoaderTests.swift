...
//  MovieQuizTests
//
//  Created by Богдан Полыгалов on 11.12.2022.
//
...
class MoviesLoaderTests: XCTestCase {
    ...
    func testSuccessLoading() throws {
        ...
        let loader = MoviesLoader(networkClient: stubNetworkClient) // cоздаём загрузчик фильмов, но использующий сетевой клиент заглушку
    ...
    func testFailureLoading() throws {
        ...
        loader.loadMovies { result in
            ...
            case .failure(let error):
                XCTAssertNotNil(error) // проверяем, что действительно ошибка, а не пустое значение
                expectation.fulfill()
            case .success(_):
                // мы не ожидаем, что пришли данные; если пришли, надо будет провалить тест
                XCTFail("Unexpected failure")
            }
        }
